...
@Component({
    selector: 'jhi-lot-stock-utility',
    templateUrl: './lot-stock-utility.component.html'
})
export class LotStockUtilityComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/lot-stock-utility'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/lot-stock-utility', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: LotStockUtility) {
        return item.id;
    }
