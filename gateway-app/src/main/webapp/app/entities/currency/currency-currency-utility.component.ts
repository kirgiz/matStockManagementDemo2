...
@Component({
    selector: 'jhi-currency-currency-utility',
    templateUrl: './currency-currency-utility.component.html'
})
export class CurrencyCurrencyUtilityComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/currency-currency-utility'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/currency-currency-utility', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: CurrencyCurrencyUtility) {
        return item.id;
    }
