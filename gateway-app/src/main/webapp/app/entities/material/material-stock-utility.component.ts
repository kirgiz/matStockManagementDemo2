...
@Component({
    selector: 'jhi-material-stock-utility',
    templateUrl: './material-stock-utility.component.html'
})
export class MaterialStockUtilityComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/material-stock-utility'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/material-stock-utility', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: MaterialStockUtility) {
        return item.id;
    }
